...
class SaleOrderLine(models.Model):
    ...
    def _get_ms_sequence(self):
        for record in self:
            section_id = 0
            if (record.display_type != 'line_section'): section_id = record.id
            else:
                if (record.section_id.id) and (record.sequence): section_id = record.section_id.id
                if (record.new_section_id.id) and (record.sequence): section_id = record.new_section_id.id
