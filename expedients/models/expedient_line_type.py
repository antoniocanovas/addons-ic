...
class ExpedienteLineType(models.Model):
    _name = 'expedient.line.type'
    _description = 'Line file types.'

    type_id = fields.Many2one('expedient.type',string='Tipo')
    procedure_id = fields.Many2one('expedient.procedure',
                                 domain=[('store','=',False)],string='Tramite',required=True)
    dependency_ids = fields.Many2many('expedient.procedure',
