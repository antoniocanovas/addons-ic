...
class ProjectRoadmap(models.Model):
    ...
    display_name = fields.Char(
        string="Nombre mostrado", store=False, compute="_compute_name"
    )
    ...
    partner_id = fields.Many2one(
        related="project_id.partner_id", string="Cliente", copy=True, tracking=True
    )
    ...
    roadmap_user_avatar = fields.Binary(
        string="Avatar", related="user_id.partner_id.image_128"
    )
    ...
    def _compute_roadmap_state(self):
        for record in self:
            state = "New"
    ...
    state = fields.Char(
        string="Estado", compute="_compute_roadmap_state", store=True, default="New"
    )
